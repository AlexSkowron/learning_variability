...
def ibis(actions, rewards, choices, idx_blocks, subj_idx, apply_rep_bias, apply_weber_decision_noise, curiosity_bias, show_progress, temperature):
    ...
    Q_samples              = np.zeros([nb_samples, 2]) # means of the beta distributions           
    Qbeta_samples          = np.zeros([nb_samples, 2, 2]) # parameters of the two beta distributions
    ...
    Qbeta_samples_move   = np.zeros([nb_samples, 2, 2])
    prev_action_move = np.zeros(nb_samples)
    mean_Q           = np.zeros([T, 2]) # mean expectations of both beta distributions across particles
    prediction_err   = np.zeros(nb_samples) # based on beta distribution expectations
    ...
    for t_idx in range(T):
        ...
        if idx_blocks[t_idx]:
            Q_samples[:] = 0.5
            Qbeta_samples[:]   = 1 # means that expectation of betas = 0.5; Does this index into all array elements?
            prev_action[:] = -1
        ...
        for n_idx in range(nb_samples):
            ...
            # update step
            if actions[t_idx] == 0 and rewards[0, t_idx] == 1:
                Qbeta_samples[n_idx, 0, 0] = (1-alpha_c) * Qbeta_samples[n_idx, 0, 0] + alpha_c + 1 # update beta distribution parameters
                Qbeta_samples[n_idx, 0, 1] = (1-alpha_c) * Qbeta_samples[n_idx, 0, 1] + alpha_c
                
                prediction_err[n_idx]     = np.abs(Q_samples[n_idx, 0] - 1)
                Q_samples[n_idx, 0]       = Qbeta_samples[n_idx, 0, 0] / (Qbeta_samples[n_idx, 0, 0] + Qbeta_samples[n_idx, 0, 1])
                if not curiosity_bias:
                    Qbeta_samples[n_idx, 1, :] = (1-alpha_u) * Qbeta_samples[n_idx, 1, :] + alpha_u
                    Q_samples[n_idx, 1]       = Qbeta_samples[n_idx, 1, 0] / (Qbeta_samples[n_idx, 1, 0] + Qbeta_samples[n_idx, 1, 1])
            elif actions[t_idx] == 0 and rewards[0, t_idx] == 0:
                Qbeta_samples[n_idx, 0, 0] = (1-alpha_c) * Qbeta_samples[n_idx, 0, 0] + alpha_c # update beta distribution parameters
                Qbeta_samples[n_idx, 0, 1] = (1-alpha_c) * Qbeta_samples[n_idx, 0, 1] + alpha_c + 1
                
                prediction_err[n_idx]     = np.abs(Q_samples[n_idx, 0] - 0)
                Q_samples[n_idx, 0]       = Qbeta_samples[n_idx, 0, 0] / (Qbeta_samples[n_idx, 0, 0] + Qbeta_samples[n_idx, 0, 1])
                if not curiosity_bias:
                    Qbeta_samples[n_idx, 1, :] = (1-alpha_u) * Qbeta_samples[n_idx, 1, :] + alpha_u
                    Q_samples[n_idx, 1]       = Qbeta_samples[n_idx, 1, 0] / (Qbeta_samples[n_idx, 1, 0] + Qbeta_samples[n_idx, 1, 1])
            elif actions[t_idx] == 1 and rewards[1, t_idx] == 1:
                Qbeta_samples[n_idx, 1, 0] = (1-alpha_c) * Qbeta_samples[n_idx, 0, 0] + alpha_c + 1 # update beta distribution parameters
                Qbeta_samples[n_idx, 1, 1] = (1-alpha_c) * Qbeta_samples[n_idx, 0, 1] + alpha_c
                
                prediction_err[n_idx]     = np.abs(Q_samples[n_idx, 1] - 1)
                Q_samples[n_idx, 1]       = Qbeta_samples[n_idx, 1, 0] / (Qbeta_samples[n_idx, 1, 0] + Qbeta_samples[n_idx, 1, 1])
                if not curiosity_bias:
                    Qbeta_samples[n_idx, 0, :] = (1-alpha_u) * Qbeta_samples[n_idx, 0, :] + alpha_u
                    Q_samples[n_idx, 0]       = Qbeta_samples[n_idx, 0, 0] / (Qbeta_samples[n_idx, 0, 0] + Qbeta_samples[n_idx, 0, 1])
            elif actions[t_idx] == 1 and rewards[1, t_idx] == 0:
                Qbeta_samples[n_idx, 1, 0] = (1-alpha_c) * Qbeta_samples[n_idx, 0, 0] + alpha_c # update beta distribution parameters
                Qbeta_samples[n_idx, 1, 1] = (1-alpha_c) * Qbeta_samples[n_idx, 0, 1] + alpha_c + 1
                
                prediction_err[n_idx]     = np.abs(Q_samples[n_idx, 1] - 0)
                Q_samples[n_idx, 1]       = Qbeta_samples[n_idx, 1, 0] / (Qbeta_samples[n_idx, 1, 0] + Qbeta_samples[n_idx, 1, 1])
                if not curiosity_bias:
                    Qbeta_samples[n_idx, 0, :] = (1-alpha_u) * Qbeta_samples[n_idx, 0, :] + alpha_u
                    Q_samples[n_idx, 0]       = Qbeta_samples[n_idx, 0, 0] / (Qbeta_samples[n_idx, 0, 0] + Qbeta_samples[n_idx, 0, 1])
            else:
                raise Exception("update step failed")
        ...
        if ess < coefficient * nb_samples:
            ...
            for n_idx in range(nb_samples):
                ...
                [loglkd_prop, Q_prop, Qbeta_prop, prev_action_prop, prediction_err_prop] = get_loglikelihood(sample_p, rewards, actions, choices, idx_blocks, t_idx + 1, apply_rep_bias, apply_weber_decision_noise, curiosity_bias, temperature) 
                ...
                if (np.log(np.random.rand()) < log_ratio):
                    ...
                    Qbeta_samples_move[n_idx]   = Qbeta_prop
                    ...
                    Qbeta_samples_move[n_idx]   = Qbeta_samples[idx_traj]
            ...
            Qbeta_samples[:]   = Qbeta_samples_move
...
def get_loglikelihood(sample, rewards, actions, choices, blocks, T, apply_rep_bias, apply_weber_decision_noise, curiosity_bias, temperature):
    ...
    for t_idx in range(T):
        ...
        if actions[t_idx] == 0 and rewards[0, t_idx] == 1:
            Qbeta[0, 0] = (1-alpha_c) * Qbeta[0, 0] + alpha_c + 1 # update beta distribution parameters
            Qbeta[0, 1] = (1-alpha_c) * Qbeta[0, 1] + alpha_c
            
            prediction_err     = np.abs(Q[0] - 1)
            Q[0]       = Qbeta[0, 0] / (Qbeta[0, 0] + Qbeta[0, 1])
            if not curiosity_bias:
                Qbeta[1, :] = (1-alpha_u) * Qbeta[1, :] + alpha_u
                Q[1]       = Qbeta[1, 0] / (Qbeta[1, 0] + Qbeta[1, 1])
        elif actions[t_idx] == 0 and rewards[0, t_idx] == 0:
            Qbeta[0, 0] = (1-alpha_c) * Qbeta[0, 0] + alpha_c # update beta distribution parameters
            Qbeta[0, 1] = (1-alpha_c) * Qbeta[0, 1] + alpha_c + 1
            
            prediction_err     = np.abs(Q[0] - 0)
            Q[0]       = Qbeta[0, 0] / (Qbeta[0, 0] + Qbeta[0, 1])
            if not curiosity_bias:
                Qbeta[1, :] = (1-alpha_u) * Qbeta[1, :] + alpha_u
                Q[1]       = Qbeta[1, 0] / (Qbeta[1, 0] + Qbeta[1, 1])
        elif actions[t_idx] == 1 and rewards[1, t_idx] == 1:
            Qbeta[1, 0] = (1-alpha_c) * Qbeta[0, 0] + alpha_c + 1 # update beta distribution parameters
            Qbeta[1, 1] = (1-alpha_c) * Qbeta[0, 1] + alpha_c
            
            prediction_err     = np.abs(Q[1] - 1)
            Q[1]       = Qbeta[1, 0] / (Qbeta[1, 0] + Qbeta[1, 1])
            if not curiosity_bias:
                Qbeta[0, :] = (1-alpha_u) * Qbeta[0, :] + alpha_u
                Q[0]       = Qbeta[0, 0] / (Qbeta[0, 0] + Qbeta[0, 1])
        elif actions[t_idx] == 1 and rewards[1, t_idx] == 0:
            Qbeta[1, 0] = (1-alpha_c) * Qbeta[0, 0] + alpha_c # update beta distribution parameters
            Qbeta[1, 1] = (1-alpha_c) * Qbeta[0, 1] + alpha_c + 1
            
            prediction_err     = np.abs(Q[1] - 0)
            Q[1]       = Qbeta[1, 0] / (Qbeta[1, 0] + Qbeta[1, 1])
            if not curiosity_bias:
                Qbeta[0, :] = (1-alpha_u) * Qbeta[0, :] + alpha_u
                Q[0]       = Qbeta[0, 0] / (Qbeta[0, 0] + Qbeta[0, 1])
        else:
            raise Exception("update failed during resampling")

    return [log_proba, Q, Qbeta, prev_action, prediction_err]
