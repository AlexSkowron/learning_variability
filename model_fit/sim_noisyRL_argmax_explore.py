...
if par > 0:
    ...
    if par == 1: # alpha chosen
        ...
        param_sim=numpy.array([par_simul[par_set-1], 0.3, 1000, 0.2])
    elif par == 2: # alpha unchosen
        par_simul=numpy.arange(start=0, stop=1.1, step=0.1)
        param_sim=numpy.array([0.8, par_simul[par_set-1], 1000, 0.2])
    elif par == 3: # epsilon
        par_simul=numpy.arange(start=0, stop=1.1, step=0.1)
        param_sim=numpy.array([0.8, 0.3, 1000, par_simul[par_set-1]])
    else:
        raise ValueError('No simulation was done')
    
    OutDir = '/home/mpib/skowron/aging_learning_var/learning_variability/model_fit/results/sim_explore/noisyRL_argmax_alpha_U0_weber0/'
    sub_ind = sub + '_' + str(par) + '_' + str(par_set)
    
    print('starting sim_sub-' + sub + '_par' + str(par) + '_ite' + str(par_set))
    
elif par == 0:
    param_sim=numpy.array([numpy.random.random_sample(size=None), 0, 1000, numpy.random.random_sample(size=None)])
    #param_sim=numpy.array([numpy.random.random_sample(size=None), numpy.random.random_sample(size=None), 1000, numpy.random.random_sample(size=None)]) 
    # ranges: alpha chosen [0,1), alpha unchosen [0,1), epsilon [0,1)
    
    OutDir = '/home/mpib/skowron/aging_learning_var/learning_variability/model_fit/results/sim_explore_all/noisyRL_argmax_alpha_U0_weber0/'
...
sim_obj.traj_param={'noise': 1, 'apply_rep': 0, 'apply_weber': 0, 'beta_softmax': 3, 'condition': 1}
...
# noisyRL argmax
r_obj.do_inference(noise=1, apply_rep = 0, apply_weber = 0, condition=1, beta_softmax=3, alpha_unchosen=0, show_progress=show_prg)
r_obj.get_map()
