...
if par > 0:
    ...
    if par == 1: # alpha chosen
        ...
        param_sim=numpy.array([par_simul[par_set-1], 0.3, 20, 2])
    elif par == 2: # alpha unchosen
        par_simul=numpy.arange(start=0, stop=1.1, step=0.1)
        param_sim=numpy.array([0.8, par_simul[par_set-1], 20, 2])
    elif par == 3: # beta unchosen
        par_simul=numpy.arange(start=0.1, stop=110.1, step=10)
        param_sim=numpy.array([0.8, 0.3, par_simul[par_set-1], 2])
    elif par == 4: # eta
        par_simul=numpy.arange(start=-10, stop=10.1, step=2)
        param_sim=numpy.array([0.8, 0.3, 20, par_simul[par_set-1]])
    else:
        raise ValueError('No simulation was done')
    
    OutDir = '/home/mpib/skowron/aging_learning_var/learning_variability/model_fit/results/sim_explore/noiselessRL_rep_alphaU03/'
    sub_ind = sub + '_' + str(par) + '_' + str(par_set)
    
    print('starting sim_sub-' + sub + '_par' + str(par) + '_ite' + str(par_set))
    
elif par == 0:
    param_sim=numpy.array([numpy.random.random_sample(size=None), 0.3, 50 * numpy.random.random_sample(size=None), 20 * numpy.random.random_sample(size=None) - 10])
    #param_sim=numpy.array([numpy.random.random_sample(size=None), numpy.random.random_sample(size=None), 50 * numpy.random.random_sample(size=None), 20 * numpy.random.random_sample(size=None) - 10]) 
    # ranges: alpha chosen [0,1), alpha unchosen [0,1), beta [0,50), eta [-10,10)
    
    OutDir = '/home/mpib/skowron/aging_learning_var/learning_variability/model_fit/results/sim_explore_all/noiselessRL_rep_alphaU03/'
...
sim_obj.traj_param={'noise': 0, 'apply_rep': 1, 'apply_weber': 0, 'beta_softmax': -1, 'condition': 0}
...
# exact RL
r_obj.do_inference(noise=0, apply_rep = 1, apply_weber = 0, condition=0, beta_softmax=-1, alpha_unchosen=0.3, show_progress=show_prg)
r_obj.get_map()
